...
[mode.main.binding]
    ...
    alt-slash = 'exec-and-forget /bin/bash -lc "~/.config/hyprspace/cheatsheet.sh"'
...
# Chromium browsers have an "app mode" (--app=URL) that opens a URL in its own
# plain window — no tab strip, no toolbar — which is what makes the cheatsheet
# read as a popup rather than as a browser.
CHEAT_BROWSER=""
for b in "Google Chrome" "Brave Browser" "Microsoft Edge" "Chromium" "Vivaldi"; do
  if [[ -d "/Applications/$b.app" ]]; then
    CHEAT_BROWSER="$b"
    ok "will open as a centred floating window via $b app mode"
    break
  fi
done
if [[ -z "$CHEAT_BROWSER" ]]; then
  warn "no Chromium browser found — the cheatsheet opens in a normal tab"
  info "install any Chromium browser to get the floating popup"
fi
...
#
#   1. open it in app mode, so it is a standalone window and not a browser tab
#   2. float it, because AeroSpace tiles it the moment it appears
#   3. resize and centre it, because AeroSpace keeps whatever size the window
#      had at the moment it was floated — by then it is already stretched
#
# An [[on-window-detected]] float rule cannot do step 2: AeroSpace evaluates
# those rules the instant the window is created, and a browser window has no
# title yet at that point (the page hasn't loaded), so a title match never
# fires — and matching on app-id would float every browser window.
write_file "$CHEAT_DIR/cheatsheet.sh" <<CHEATLAUNCH
#!/usr/bin/env bash
# Opens the hyprspace keybind cheatsheet as a centred floating window.
# Generated by hyprspace.sh — edit there, not here.
set -uo pipefail

URL="file://\$HOME/.config/hyprspace/keybinds.html"
BROWSER="${CHEAT_BROWSER}"
W=940
H=700

if [ -z "\$BROWSER" ]; then
  # No Chromium browser: fall back to a plain tab. It won't float.
  open "\$URL"
  exit 0
fi

open -na "\$BROWSER" --args --app="\$URL" --window-size=\$W,\$H

# Wait for the window to take focus, then float it. Bounded so a failure to
# appear costs ~5s rather than hanging forever.
for _ in \$(seq 1 25); do
  sleep 0.2
  case "\$(aerospace list-windows --focused --format '%{window-title}' 2>/dev/null)" in
    *"hyprspace keybinds"*) aerospace layout floating >/dev/null 2>&1; break ;;
  esac
done

# Now give it a real popup size, centred on the screen.
#
# The browser name is baked in as a literal rather than passed as an argument:
# 'tell application someVariable' stops AeroSpace resolving the app's
# scripting terminology at compile time, and 'title of win' then fails with
# -1700. All Chromium browsers share this terminology, so a literal is fine.
sleep 0.3
osascript >/dev/null 2>&1 <<APPLESCRIPT || true
tell application "Finder" to set d to bounds of window of desktop
set w to \$W
set h to \$H
set x to ((item 3 of d) - w) / 2
set y to ((item 4 of d) - h) / 2
tell application "${CHEAT_BROWSER}"
  repeat with win in windows
    if title of win contains "hyprspace keybinds" then
...
APPLESCRIPT
CHEATLAUNCH
run chmod +x "$CHEAT_DIR/cheatsheet.sh"
